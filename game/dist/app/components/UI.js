...
const UI = (ctx, canvas) => {
    ...
    const { get } = Contract();
    const drawTile = (position, offset) => {
    ...
    const counter = () => {
        ...
        ctx.fillText(text, canvas.width - tileSize - metrics.width / 2, tileSize - (metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent) / 2);
    };
    const itemsBar = (inventory) => {
        const speed = 0.003;
        const amplitude = 8;
        const waveStep = 0.5;
        items.length = 0;
        for (let x = 0; x < grid.ui.inventory.length; x++) {
            for (let y = 0; y < grid.ui.inventory[x].length; y++) {
                ...
                if (!cell)
                    continue;
                drawTile(new Vector2(canvas.width / 2 - grid.ui.inventory.length * tileSize + tileSize * y, canvas.height - (grid.ui.inventory[x].length / 2) * tileSize + tileSize * x), new Vector2(tiles[cell].x, tiles[cell].y));
        ...
        if (Object.values(inventory).length && items.length) {
            ...
            for (let i = 0; i < inventoryToArray.length; i++) {
                ...
                const coordinate = new Vector2(canvas.width / 2 - grid.ui.inventory.length * tileSize + tileSize * cell.coordinate.y, canvas.height -
                    24 -
                ...
                if (inventoryToArray[i].type === 'amethysts' || inventoryToArray[i].level) {
                    ...
                    ctx.textAlign = 'center';
                    ctx.textBaseline = 'middle';
                    ctx.fillText(inventoryToArray[i].level
                        ? `${inventoryToArray[i].level} Lv`
                        : inventoryToArray[i].count.toString(), coordinate.x + tileSize / 2, coordinate.y + tileSize * 2);
                }
            }
        }
    };
