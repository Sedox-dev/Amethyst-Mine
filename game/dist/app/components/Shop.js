...
const Shop = (ctx, canvas, address, ws, inventory) => {
    ...
    function buyUpgrade(itemType) {
        if (!prices[instruments[itemType]])
            return;
        if (errorMessage === 'Too far away')
            return;
        ...
        ws.listen(`upgrade_${itemType}_result`).then((res) => {
    ...
    return {
        ...
        render: function (position) {
            ...
            if (!address || address === 'guest') {
                ...
                ctx.fillText('Available with a connected crypto wallet', canvas.width / 2, canvas.height / 2);
                return;
            }
            ctx.textAlign = 'center';
            ctx.fillStyle = 'White';
            ...
            const shortAddress = address && address !== 'guest'
                ? `${address.slice(0, 6)}...${address.slice(-4)}`
                : 'NOT CONNECTED';
            ctx.fillText(`Wallet: ${shortAddress}  |  Amethysts: ${amethysts !== null ? amethysts : '0'}`, centerX, layout.modal.y + 60);
            ...
            ctx.lineWidth = 2;
            ctx.beginPath();
            ctx.roundRect(layout.inputRect.x, layout.inputRect.y, layout.inputRect.w, layout.inputRect.h, 6);
            ctx.fill();
            ctx.stroke();
            ...
            ctx.beginPath();
            ctx.roundRect(layout.pickaxeCard.x, layout.pickaxeCard.y, layout.pickaxeCard.w, layout.pickaxeCard.h, 6);
            ctx.fill();
            ...
            ctx.fill();
            if (textures) {
                ctx.drawImage(image, 48, 16, 16, 16, layout.pickaxeCard.x + 10, layout.pickaxeCard.y + 10, 56, 56);
            }
            ctx.fillStyle = 'White';
            ctx.font = 'bold 13px monospace';
            ctx.fillText(`Pickaxe Lv${+instruments.pickaxe + 1}`, layout.pickaxeCard.x + 75, layout.pickaxeCard.y + 20);
            ctx.fillStyle = 'White';
            ctx.beginPath();
            ctx.roundRect(layout.pickaxeBtn.x, layout.pickaxeBtn.y, layout.pickaxeBtn.w, layout.pickaxeBtn.h, 4);
            ctx.fill();
            ctx.textAlign = 'center';
            ctx.fillStyle = '#0a0a0f';
            ctx.font = 'bold 12px monospace';
            ctx.fillText(!prices[instruments.pickaxe] ? 'Max' : `Buy [${prices[instruments.pickaxe]} AME]`, layout.pickaxeBtn.x + layout.pickaxeBtn.w / 2, layout.pickaxeBtn.y + 18);
            ctx.textAlign = 'left';
            ctx.fillStyle = '#14141a';
            ctx.beginPath();
            ctx.roundRect(layout.swordCard.x, layout.swordCard.y, layout.swordCard.w, layout.swordCard.h, 6);
            ctx.fill();
            ctx.fillStyle = '#252530';
            ctx.beginPath();
            ...
            ctx.fill();
            if (textures) {
                ctx.drawImage(image, 48, 32, 16, 16, layout.swordCard.x + 10, layout.swordCard.y + 10, 56, 56);
            }
            ctx.fillStyle = 'White';
            ctx.font = 'bold 13px monospace';
            ctx.fillText(`Sword Lv${+instruments.sword + 1}`, layout.swordCard.x + 75, layout.swordCard.y + 20);
            ctx.fillStyle = 'White';
            ctx.beginPath();
            ...
            ctx.fillStyle = '#0a0a0f';
            ctx.font = 'bold 12px monospace';
            ctx.fillText(!prices[instruments.sword] ? 'Max' : `Buy [${prices[instruments.sword]} AME]`, layout.swordBtn.x + layout.swordBtn.w / 2, layout.swordBtn.y + 18);
